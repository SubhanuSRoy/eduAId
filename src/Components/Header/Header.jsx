...
// import Logo from "../../logo/RM_logo.png";
import DropdownUser from "../Common/DropdownUser";

const Header = ({ sidebarOpen, setSidebarOpen }) => {
  return (
    <header className="sticky top-0 z-999 flex w-full text-gray-900 sm:text-white bg-white sm:bg-boxdark-2 drop-shadow-none">
          ...
          >
            <span className="relative block h-5.5 w-5.5 cursor-pointer">
              ...
              <span className="absolute right-0 h-full w-full rotate-45">
                <span
                  className={`absolute left-2.5 top-0 block h-full w-0.5 rounded-sm bg- delay-300 duration-200 ease-in-out bg-white ${
                    !sidebarOpen && "!h-0 !delay-[0]"
                  }`}
                ></span>
                <span
                  className={`delay-400 absolute left-0 top-2.5 block h-0.5 w-full rounded-sm bg- duration-200 ease-in-out bg-white ${
